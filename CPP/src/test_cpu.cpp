...
{  
    ...
    double cpu_idle = idle / sum;  
    double use = n->user + n->system + n->nice - o->user - o->system - o->nice;  
    double cpu_use = use / sum;  
    printf("CPU IDLE percent: %.1f\%; Used percent: %.1f\%\n",cpu_idle*100.0F, cpu_use*100.0F);  
}  
...
{  
    ...
    while(true) {
        ...
        printf("memory percent: %.1f%\n", mem_stat.MemFree * 100.0 / ( mem_stat.MemTotal * 1.0  ) );
        //第一次获取cpu使用情况  
