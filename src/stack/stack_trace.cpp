...
std::vector<std::string> StackTrace::get_stack_trace(int frames_skip, void *secret) {
	...
	if (module) {
		...
		if (ptrRtlCaptureStackBackTrace)
			num_frames = ptrRtlCaptureStackBackTrace(frames_skip + 1, max_frames_, frames, (DWORD *)out_hash);
		FreeLibrary(module);
	}
    ...
    _Unwind_Word stacks[max_frames_];
    size_t num_frames = 0;
#ifndef __GLIBC__
    if (secret != nullptr) {
        ...
        while (num_frames < max_frames_ && fp != nullptr) {
...
#else
    void *frames[max_frames_];
    std::memset(frames, 0, (sizeof(void *)) * max_frames_);
    int num_frames = backtrace(frames, max_frames_);
    ...
    for (int cnt = 0; cnt < num_frames; cnt++) {
        ...
        backtrace_text.push_back(fmt::format("{} ({})", StringHelp::get_file_name(filename), func_nume));
    }
